...
//use loader::binary::{BinaryLoader, JsonLoader};
...
fn build_guarded_region(
    ...
    if let Some(l) = lopt {
        let data_start_addr = l.move_sec_aslr(region_addr, prot);
        l.move_all(region_addr, String::from(format!("{}_update",uk_name)), data_start_addr);
    } /*else {
        let mut bin = BinaryLoader::new();
        bin.read_uk_file(String::from(format!("{}_update_minimal",uk_name)));
        let json_loader =
        JsonLoader::read_sections(String::from(format!("{}_minimal.sec",uk_name)));
        let l = LoaderInfo::new(bin, json_loader, false);
        l.move_sec_aslr(region_addr, prot);
        l.move_all(region_addr);
    }*/
...
pub fn create_guest_memory(
    ...
    for region in regions {
        //println!("\n[DEV] region: {:?} {:?} {}", region.0, region.1, region.2);
