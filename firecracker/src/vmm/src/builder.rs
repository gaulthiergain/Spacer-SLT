...
use loader::binary::{BinaryLoader, SectionsLoader};
use loader::uk_loader::LoaderInfo;
...
pub fn build_microvm_for_boot(
    ...
    let request_ts_cpy = TimestampUs::default();

    let boot_config = vm_resources.boot_source().ok_or(MissingKernelConfig)?;

    let mut bin = BinaryLoader::new();
    let mut use_aslr = false;
    if boot_config.description.kernel_image_path.to_owned().contains("_aslr"){
        bin.read_uk_file(boot_config.description.kernel_image_path.to_owned() + "_update");
        use_aslr=true;
    }
    let l = LoaderInfo::new(bin, SectionsLoader::new(boot_config.description.kernel_image_path.to_owned() + ".sec"), use_aslr);

    let track_dirty_pages = vm_resources.track_dirty_pages();
    let guest_memory = create_guest_memory(
        vm_resources.vm_config().mem_size_mib,
        track_dirty_pages,
        &boot_config.description.kernel_image_path.to_owned(),
        Some(&l),
    )?;
    let vcpu_config = vm_resources.vcpu_config();

    let entry_addr = GuestAddress(0x109000); //todo check
    ...
    let boot_time_us = now_tm_us.time_us - request_ts_cpy.time_us;
    let boot_time_cpu_us = now_tm_us.cputime_us - request_ts_cpy.cputime_us;
    info!(
        "FC-boot-time = {:>6} us {} ms, {:>6} CPU us {} CPU ms",
        boot_time_us,
        boot_time_us / 1000,
        boot_time_cpu_us,
...
/*fn load_kernel(
    ...
    println!("[DEV] EntryAddr {:#04x}", entry_addr.kernel_load.0); // 0x109000 - _libkvmplat_start64
