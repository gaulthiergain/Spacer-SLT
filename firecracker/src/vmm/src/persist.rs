...
pub fn restore_from_snapshot(
    ...
    /*use std::mem;
    //microvm_state.vcpu_states[0].regs.rip = 0x1d2e84;
    println!("[DEBUG_FG] microvm_state vm_info: {:?}", microvm_state.vm_info.mem_size_mib);
    println!("[DEBUG_FG] microvm_state memory_state regions: {:?} (sizeof) {:?}", microvm_state.memory_state.regions, mem::size_of::<GuestMemoryState>());
    println!("[DEBUG_FG] microvm_state vm_state (sizeof): {:?}", mem::size_of::<VmState>());
    println!("[DEBUG_FG] microvm_state vcpu_states cpuid: {:x?}", get_vendor_id_from_cpuid(&microvm_state.vcpu_states[0].cpuid));
    println!("[DEBUG_FG] microvm_state vcpu_states kvm regs: {:x?}", microvm_state.vcpu_states[0].regs);*/
    ...
    let uk_name = &params.uk_name;
    let (guest_memory, uffd) = match params.mem_backend.backend_type {
        MemBackendType::File => (
            guest_memory_from_file(mem_backend_path, mem_state, track_dirty_pages, uk_name)?,
            None,
        ),
...
fn guest_memory_from_file(
    ...
    GuestMemoryMmap::restore(Some(&mem_file), mem_state, track_dirty_pages, uk_name)
...
fn guest_memory_from_uffd(
    ...
    let guest_memory =
        GuestMemoryMmap::restore(None, mem_state, track_dirty_pages, &String::new()).map_err(DeserializeMemory)?;
