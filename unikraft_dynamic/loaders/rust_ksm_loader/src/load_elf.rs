...
impl ElfLoad {
    ...
    pub fn load(load_info: &LoadInfo) {
        ...
        */
        ...
        // Mmap 128 MB of memory
        let memory_mib = 16;
        let total_mapping_size0 = Self::get_total_mapping_size(&segments);
        let total_mapping_size = memory_mib * 1024 * 1024;
        if total_mapping_size0 > total_mapping_size {
            panic!("The binary is too large to be loaded into the allocated memory");
        }
        ...
        let load_addr = unsafe {
            //println!("Mapping {} bytes", total_mapping_size);
            //println!("segments[0].virt_addr: {:x}", segments[0].virt_addr);
            mmap(segments[0].virt_addr as *mut c_void, total_mapping_size, prot_flags, map_flags, -1, 0).expect("Failed to map!")
        };

        unsafe {
            libc::madvise(load_addr, total_mapping_size, MADV_MERGEABLE);
        }
        ...
        for seg in segments.iter() {
            ...
            unsafe {
                //println!("Copying {} bytes to {:x}", seg.data.len(), addr);
