...
class Unikernel:
    ...
    def create_param_files(self, libname, newsrc):
        ...
        # Create a new dir which contains libparam
    ...
    def update_loc_counter(self, subset, type_maps, dce, snapshot, compact):

        array_sect = [".text"]
        if snapshot:
            array_sect = [".text", ".rodata", ".data", ".bss"]
        
        if compact:
            array_sect = [".text", ".rodata"]

        if type_maps not in self.sb_link:
            self.sb_link[type_maps] = StringBuilder()
            ...
            if ukLib.name in self.objects:
                ...
                for type_sect in array_sect:
                    ...
                    if ".text" in type_sect:
                        ...
                        if compact:
                            if dce and "app" in ukLib.name and ukLib.occurence == 1:
                                self.sb_link[type_maps].append(".text.rodata.{} 0x{:x} : {{\n\t{}{}(.text);\n\t{}{}(.text.*); /* 0x{:x} */\n\t{}{}(.rodata);\n\t{}{}(.rodata.*); /* 0x{:x} */\n}}\n".format(ukLib.name, self.loc_counter, ukLib.name, OBJ_EXT, ukLib.name, OBJ_EXT, ukLib.total_size[".text"], ukLib.name, OBJ_EXT, ukLib.name, OBJ_EXT, ukLib.total_size[".rodata"]))
                            else:
                                self.sb_link[type_maps].append(".text.rodata.{} 0x{:x} : {{\n\t{}{}(.text); /* 0x{:x} */\n\t{}{}(.rodata); /* 0x{:x} */\n}}\n".format(ukLib.name, self.loc_counter, ukLib.name, OBJ_EXT, ukLib.total_size[".text"], ukLib.name, OBJ_EXT, ukLib.total_size[".rodata"]))
                            #self.loc_counter = round_to_n(self.loc_counter, ukLib.sections[type_sect].addralign)
                        else:
                            self.sb_link[type_maps].append(".text.{} 0x{:x} : {{\n\t{}{}(.text);\n\t{}{}(.text.*); /* 0x{:x} */\n}}\n".format(ukLib.name, self.loc_counter, ukLib.name, OBJ_EXT, ukLib.name, OBJ_EXT, ukLib.total_size[".text"]))
                            
                            self.sb_rodata[ukLib.name] = ukLib.total_size[".rodata"]
                            self.total_rodata_size += ukLib.total_size[".rodata"]
                            #self.sb_link[type_maps].append(".rodata.{} : {{\n\t{}{}(.rodata);\n\t{}{}(.rodata.*); /* 0x{:x} */\n}}\n".format(ukLib.name, ukLib.name, OBJ_EXT, ukLib.name, OBJ_EXT, ukLib.total_size[".rodata"]))
                            
                    
                    if snapshot and ".data" in type_sect:
                        self.loc_counter = round_to_n(self.loc_counter, PAGE_SIZE)
                        if dce and "app" in ukLib.name and ukLib.occurence == 1:
                            self.sb_link[type_maps].append(".data.bss.{} 0x{:x} : {{\n\t{}{}(.data);\n\t{}{}(.data.*); /* 0x{:x} */\n\t{}{}(.bss);\n\t{}{}(.bss.*); /* 0x{:x} */\n}}\n".format(ukLib.name, self.loc_counter, ukLib.name, OBJ_EXT, ukLib.name, OBJ_EXT, ukLib.total_size[".data"], ukLib.name, OBJ_EXT, ukLib.name, OBJ_EXT, ukLib.total_size[".bss"]))
                        else:
                            self.sb_link[type_maps].append(".data.bss.{} 0x{:x} : {{\n\t{}{}(.data); /* 0x{:x} */\n\t{}{}(.bss); /* 0x{:x} */\n}}\n".format(ukLib.name, self.loc_counter, ukLib.name, OBJ_EXT, ukLib.total_size[".data"], ukLib.name, OBJ_EXT, ukLib.total_size[".bss"]))
    ...
    def process_file_dce(self, path, libname, s_name):
        ...
        with open(path + libname, 'rb') as f:
            ...
            for s in s_name:
                ...
                ukLib.sections[s]= UkSection(s, total_size, sec["sh_addr"], sec["sh_offset"], sec["sh_addralign"])
                self.increment_sect(ukLib.sections[s], ukLib)
    ...
    def process_build_folder(self, path, global_maps, objs_files, update=True, dce=False):
        ...
        for lib in sorted(os.listdir(path)):
            if "x86_64" not in lib and OBJ_EXT in lib:
                ...
                if dce and "app" in libname:
                    if "dev/base_instances" in self.workspace:
                        src_file = os.path.join(self.workspace.replace("spacer-aggregated","dce"), "build", libname + OBJ_EXT)
                        src_file = os.path.join(self.workspace.replace("spacer","dce"), "build", libname + OBJ_EXT)
                    else:
                        src_file = os.path.join(self.workspace.replace("apps","apps_size"), "build", libname + OBJ_EXT)
                    dst_file = os.path.join(self.workspace, "build", libname + OBJ_EXT)
                    
                    logger.debug("Copy {} to {}".format(src_file, dst_file))
                    shutil.copyfile(src_file, dst_file)
                    ukLib = self.process_file_dce(path, lib, sec_name)
                else:
