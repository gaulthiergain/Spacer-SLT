...
def process_instruction(uk, ins, s, used_addr, optimized_suit):
    ...
    if addrInt == 0xffffff or len(used_addr) > 18:
        ...
        if any(ins.address + offset in diff_addr for offset in range(ins.size)):
            ...
            if x != None:
                ...
                if "libvfscore" in uk_sect.name and len(ins.bytes) > 5 and 0x48 == ins.bytes[0] and 0x8d == ins.bytes[1] and (0x88 == ins.bytes[2] or 0x90 == ins.bytes[2]) and 0xFF == ins.bytes[5]:
                    ...
                    continue
                printv("[X IS NONE]: 0x{:x} {:<32}{:<20}{:<32}".format(ins.address, ' '.join(re.findall('..',ins.bytes.hex())), ins.mnemonic, ins.op_str))
                bt.extend(ins.bytes)
        else:
...
def diff_compare_bytes(uks, sect_name):
    ...
    for index_sect in range(0, len(same_sects)-1):
        ...
        for i in range(0, same_sects[index_sect].size):
            try:
                e1 = same_sects[index_sect].content[i]
                e2 = same_sects[index_sect+1].content[i]
            except IndexError:
                print("[WARNING] IndexError for section {} at index {:x} - size1: {:x} - size2: {:x}".format(sect_name, i, len(same_sects[index_sect].content), len(same_sects[index_sect+1].content)))
                continue
...
def rewrite_uk_v(l_uks, json_file, v, workspace, rewrite_all=False):
    ...
    for uk_path in l_uks:
        ...
        uks.append(uk)
        map_uks[uk_path] = uk
        excluded_sections=[".text.rodata.apphelloworldgo"] #".text.rodata.libcxx", ".text.rodata.libcxxabi", ".text.rodata.libgo"]
        
        for _, s in enumerate(uk.sections):
            if s.name in excluded_sections:
                print("[WARNING] Skip section {} from {}".format(s.name, uk.name))
                continue
             
            if s.size == 0:
                printv("[WARNING] Section {} has size 0 -> SKIP".format(s.name))
            elif rewrite_all and "app" in s.name and ".text" in s.name:
