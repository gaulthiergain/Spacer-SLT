...
APP="apps_size"
UKS_INCLUDED=["lib-helloworld", "lib-proxy"]
...
class AslrManager:
    def __init__(self, args):
        ...
        self.is_dce = "size" in args.app_folder
    ...
    def relink(self, path, use_vfscore, kvm_plat):           
        ...
        cmd = 'gcc -nostdlib -Wl,--omagic -Wl,--build-id=none -nostdinc -no-pie -Wl,-m,elf_x86_64 -Wl,-m,elf_x86_64 -Wl,-dT,{}/lib{}plat/link64_aslr.lds -Wl,-T,{}/lib/uksched/extra_aslr.ld {} -o unikernel_{}-x86_64_aslr.dbg'.format(path, kvm_plat, self.unikraft_path, linker_add, kvm_plat)
        if self.is_dce:
            cmd = 'gcc  -nostdlib -Wl,--omagic -Wl,--build-id=none  -nostdinc -Wl,--gc-sections -no-pie -Wl,-m,elf_x86_64 -Wl,-m,elf_x86_64 -Wl,-dT,{}/lib{}plat/link64_aslr.lds -Wl,-T,{}/lib/uksched/extra_aslr.ld {} -o unikernel_{}-x86_64_aslr.dbg'.format(path, kvm_plat, self.unikraft_path, linker_add, kvm_plat)
        
        print(cmd)
        p = subprocess.run(cmd, shell=True)
        if p.returncode == 0:
            p = subprocess.run("cp unikernel_{}-x86_64_aslr.dbg unikernel_{}-x86_64_aslr".format(kvm_plat, kvm_plat), shell=True)
            p = subprocess.run("strip unikernel_{}-x86_64_aslr".format(kvm_plat), shell=True)
        ...
        for uk in self.uks:
            ...
            if self.is_dce and self.same_mapping:
                ...
                for app_lib in uk.objects:
                    ...
                    libs.append("{}.text.{} : {{ {}{}(.text);{}{}(.text.*); }}\n".format(offset,app_lib, app_lib, OBJ_EXT, app_lib, OBJ_EXT))
            ...
            with open(os.path.join(path, plat, "link64.lds"), "r") as file_in, open(os.path.join(path, plat, "link64_aslr.lds"), "w") as file_out:
                file_out.write(self.process_link64_aslr(file_in.read().splitlines()))
                logger.info("Written link64_aslr.lds in {}/ ".format(path + "/" + plat))
    ...
    def second_pass_dce(self):
        ...
        for uk in self.uks:
            ...
            binary  = lief.parse("{}/unikernel_{}-x86_64_aslr".format(os.path.join(self.workspace, uk.name, "build"), uk.kvm_plat))
