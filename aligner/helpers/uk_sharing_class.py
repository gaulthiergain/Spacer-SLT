...
class Unikernel:
    def __init__(self, name):
        self.shortname = self.filter_name(name)
        self.name = name
        self.type_unikernel = ALIGN
        self.segments = list()
        self.sections = list()
    ...
    def filter_name(self, name):
        if "/" in name:
            return name.split("/")[5].strip(DBG_EXTENSION)
        return name
...
class Section:
    def __init__(self, name, start, offset, size, alignment, uk_name, data=None):
        self.name = name
        self.start = start
