...
pub fn create_guest_memory(
    ...
    for region in regions {
        ...
        let mmap_region =
            build_guarded_region(region.0.clone(), region.2, prot, flags, track_dirty_pages, uk_name)
...
mod tests {
    ...
    fn test_build_guarded_region() {
        // Create anonymous guarded region.
        {
            ...
            let region = build_guarded_region(None, size, prot, flags, false, &"".to_string()).unwrap();
    ...
    fn test_create_guest_memory() {
        // Test that all regions are guarded.
        {
            ...
            let guest_memory = create_guest_memory(&regions, false, &"".to_string()).unwrap();
        ...
        // Check dirty page tracking is on.
        {
            ...
            let guest_memory = create_guest_memory(&regions, true, &"".to_string()).unwrap();
            guest_memory.iter().for_each(|region| {
